from analyser import TextMetricsAnalyzer
from indices import FleschKincaidGradeLevel, GunningFogIndex
import sys
...
if __name__ == "__main__":
    try:
        ...
        fk_value, fk_interpretation = fk_index.calculate_and_interpret(
            text_metrics)
        gf_value, gf_interpretation = gf_index.calculate_and_interpret(
            text_metrics)

        print(f"Flesch-Kincaid Grade Level Index: {fk_value}")
        print(
            f"Flesch-Kincaid Grade Level Interpretation: {fk_interpretation}")
        print()
